...
void apply_periodic_boundaries(number_type *field, int width, int height) {
  ...
  // periodic boundaries must also be on the inside boundaries
}
...
void game(int width, int height, int num_timesteps, int *decomposition) {
  ...
  int starts[2]; //hier selber rumspielen
  int ends[2]; // hier auch ggf. checker mit 7 ? -> Methode finden, wie sinnvoll aufteilen (gerade Zahlen verlangen, restlos teilbar, etc.?)
  int time = 0;
  starts[X] = 1;
  starts[Y] = 1;
  ends[X] = width - 1;
